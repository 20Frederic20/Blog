...
class Promotion(models.Model):
    ...
    year_start = models.IntegerField(_('Year start'), choices=YEAR_CHOICES, default=current_year)
    year_end = models.IntegerField(_('Year end'), choices=YEAR_CHOICES, default=current_year)
...
class Classroom(models.Model):
    ...
    code = models.CharField(_("Code de la classe"), max_length=9, choices=CLASSROOMS, unique=True)
...
class Coefficient(models.Model):
    subject = models.ForeignKey(Subject, verbose_name=_("Matiere"), on_delete=models.CASCADE)
    value = models.PositiveIntegerField(_("Coefficient"))
    classroom = models.ForeignKey(Classroom, verbose_name=_("Classe"), on_delete=models.CASCADE)

    def __str__(self):
        return '{}-{}{}-{}'.format(self.matiere, self.classe.code, self.valeur)
...
class Instruct(models.Model):
    ...
    class Meta:
        db_table = 'instructs'
        unique_together=('user', 'promotion', 'classroom', 'subject')
        managed = True
        verbose_name = 'Instcruct'
